...
test.describe('Curve Layout tool', () => {
  ...
  test('the bend value field sets a curvature by number', async ({ page }) => {
    const before = await readLayoutJson(page)
    await openCurveLayout(page)

    const field = page.locator(SELECTORS.CURVE_LAYOUT.BEND_VALUE)
    await field.fill('12.5')
    await canvasHelper.waitForCanvasStability()

    const typed = await readLayoutJson(page)
    expect(typed).not.toEqual(before)
    // The slider follows the field, so the two controls describe the same curve.
    await expect(page.locator(SELECTORS.CURVE_LAYOUT.BEND)).toHaveValue('12.5')

    // A value the 1% slider could never land on is a different curve, not a rounded one.
    await field.fill('13')
    await canvasHelper.waitForCanvasStability()
    expect(await readLayoutJson(page)).not.toEqual(typed)
  })

  test('dragging a handle reads back into the bend field', async ({ page }) => {
    await openCurveLayout(page)
    await expect(page.locator(SELECTORS.CURVE_LAYOUT.BEND_VALUE)).toHaveValue('0')

    await dragHandle(page, 1, 0, -110)
    await canvasHelper.waitForCanvasStability()

    await expect
      .poll(async () => Number(await page.locator(SELECTORS.CURVE_LAYOUT.BEND_VALUE).inputValue()))
      .not.toBe(0)
  })

  test.describe('the tool owns the canvas while it is open', () => {
    test('the Extra Tools entry cannot re-open it over a live preview', async ({ page }) => {
      // Re-opening used to snapshot the preview as the original, leaving Cancel nowhere to go.
      const before = await readLayoutJson(page)
      await openCurveLayout(page)
      await dragHandle(page, 1, 0, -110)
      await canvasHelper.waitForCanvasStability()
      ...
      await page.locator(EXTRA_TOOLS_BUTTON).click()
      await expect(page.locator('.dropdown-item', { hasText: 'Curve Layout' })).toBeDisabled()
      await page.keyboard.press('Escape')
    ...
    test('undo does not fire while a preview is live', async ({ page }) => {
      await openCurveLayout(page)
      await dragHandle(page, 1, 0, -110)
      await canvasHelper.waitForCanvasStability()
      const bent = await readLayoutJson(page)

      await page.locator(SELECTORS.CANVAS.MAIN).press('Control+z')
      await canvasHelper.waitForCanvasStability()
    ...
    test('Escape in the Gap field dismisses the field, not the edit', async ({ page }) => {
      await openCurveLayout(page)
      await dragHandle(page, 1, 0, -110)
      await canvasHelper.waitForCanvasStability()
      const bent = await readLayoutJson(page)

      await page.locator(SELECTORS.CURVE_LAYOUT.GAP).click()
      await page.keyboard.press('Escape')

      await expect(page.locator(SELECTORS.CURVE_LAYOUT.PANEL)).toBeVisible()
      expect(await readLayoutJson(page)).toEqual(bent)

      // Focus has left the field, so the next Escape is the tool's again.
      await page.keyboard.press('Escape')
      await expect(page.locator(SELECTORS.CURVE_LAYOUT.PANEL)).toBeHidden()
    })
  })
