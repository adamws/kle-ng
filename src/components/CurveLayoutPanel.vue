    ...
    <div class="panel-content">
      ...
      <div class="panel-body">
        ...
        <!-- Bend: moves both inner handles along the normal at once, so the common case
             needs no handle dragging at all. The slider is for finding a shape, the number
             field for landing on one — a curvature worth keeping is hard to hit on a 320px
             track, and impossible to reproduce later without a number to type. -->
        <div class="controls-section mb-3">
          <div class="d-flex justify-content-between align-items-center mb-1">
            <label class="form-label small mb-0">Bend</label>
            <!-- The input sizes itself to its container, like the Gap field inside its column. -->
            <div class="bend-value">
              <CustomNumberInput
                :model-value="bend"
                data-testid="curve-layout-bend-value"
                :step="BEND_STEP"
                :ctrl-step="5"
                :min="-100"
                :max="100"
                :value-on-clear="0"
                size="compact"
                title="Curvature as a percentage of the block's own length"
                @update:model-value="updateBendValue"
              >
 ...
 * The slider and the number field each write the spine from their own input handler instead, so
 * there is no loop to guard.
 */
const bend = ref(0)

/**
 * Granularity of both bend controls, in percent.
 *
 * Half a percent is roughly a pixel of deflection at the width the panel is drawn at, so it is
 * the finest step the slider can actually express — the number field accepts any value in range,
 * this only sets what one arrow key or spinner click moves.
 */
const BEND_STEP = 0.5

/** Round a bend percentage to something worth showing in the field. */
const roundBend = (value: number): number => Math.round(value * 10) / 10

const clampBend = (value: number): number => Math.max(-100, Math.min(100, value))
...
const measureBend = (): number => {
  ...
  return clampBend(roundBend((average / scale) * 100))
}
...
/**
 * The number field's half of the same control.
 *
 * Fires on every keystroke and spinner tick, like the Gap field, so the canvas previews the
 * typed curvature as it is typed. Clearing the field reads as 0 — a straight spine — rather
 * than leaving the last curve on screen with an empty box claiming otherwise.
 */
const updateBendValue = (value: number | undefined) => {
  bend.value = clampBend(value ?? 0)
  applyBend(bend.value)
}
...
/**
 * Inputs that hold text the user is part-way through typing. Both shortcuts defer to these, and
 * to nothing else: a slider or a checkbox does nothing of its own with Enter or Escape, so
 * swallowing them there would only make the shortcuts feel unreliable.
 */
 ...
 *
 * Escape has to defer to the same test. `CustomNumberInput` answers Escape by blurring the field
 * and does not stop propagation, so without this a user dismissing the Gap field would throw the
 * whole curve away instead. The blur moves focus to the body, so a second Escape cancels.
 */
const handleKeydown = (event: KeyboardEvent) => {
  ...
  if (event.key === 'Escape') {
    ...
    handleCancel()
  } else if (event.key === 'Enter' && curveStore.canApply) {
    event.preventDefault()
    handleApply()
  }
}
...
/* Sits on the label's own line, so it has to stay narrow: wide enough for "-99.5" plus the
   suffix and the spinner, and no wider. */
.bend-value {
  width: 96px;
  flex: 0 0 auto;
}
