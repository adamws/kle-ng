<template>
  <!--
    The container tracks the canvas as it scrolls. Overlays here are positioned against
    `.keyboard-canvas-wrapper` rather than the scrolling `.keyboard-canvas-container` (which is
    not itself a positioned element), so they stay pinned to the viewport while the canvas moves
    underneath. `MatrixAnnotationOverlay` compensates the same way.
  -->
  <div
    v-if="visible"
    class="curve-layout-overlay-container"
...
/**
 * Where inside the handle the pointer went down, in overlay pixels.
 *
 * Carried through the drag so the handle moves *with* the pointer rather than snapping its
 * centre to it. The targets are 14–18px across, so without this, pressing near the edge of one
 * jumps the spine — and re-solves the whole layout — before the user has moved at all.
 */
...
const flush = () => {
  ...
  if (index === null || !pendingEvent || !overlayRef.value) {
    pendingEvent = null
    return
  }
  ...
  curveStore.setSpinePoint(index, {
    x: ((clientX - rect.left + grabOffset.x) / props.zoom - props.coordinateOffset.x) / props.unit,
    y: ((clientY - rect.top + grabOffset.y) / props.zoom - props.coordinateOffset.y) / props.unit,
  })
}
...
const stopDrag = () => {
  ...
  if (pendingFrame !== null) {
    ...
    pendingFrame = null
    // Apply the last sampled position; a drag that ends between frames must not lose its final
    // move. A no-op once the overlay is gone, which `flush` handles.
    flush()
  }
...
const startDrag = (index: number, event: PointerEvent) => {
  const rect = overlayRef.value?.getBoundingClientRect()
  const handle = screenPoints.value[index]
  grabOffset =
    rect && handle
      ? { x: handle.x - (event.clientX - rect.left), y: handle.y - (event.clientY - rect.top) }
      : { x: 0, y: 0 }
  ...
  // Deliberately no initial sample: pressing a handle should select it, not move it.
}

// Closing the tool mid-drag — Escape, Apply, or another tool taking the canvas — removes the
// SVG but not the window listeners, and leaves `isDraggingHandle` set. The canvas suppresses
// every resize while that flag is true, so it would stay frozen for the rest of the session.
watch(
  () => props.visible,
  (isVisible) => {
    if (!isVisible) stopDrag()
  },
)
...
<style scoped>
.curve-layout-overlay-container {
  position: absolute;
  left: 0;
  top: 0;
  width: 100%;
...
.curve-layout-overlay {
  ...
  top: 0;
  pointer-events: none;
  /* The spine may lead outside the canvas bounds — a curve can be dragged past the keys. */
  overflow: visible;
}
