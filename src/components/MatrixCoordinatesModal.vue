...
const handleAutomaticAnnotation = () => {
  ...
  // Surface duplicate warning if the algorithm flagged one (rotation-aware fallback case).
  ...
  // Sync to the drawing store so the overlay can render lines.
  matrixDrawingStore.clearDrawings()
  matrixDrawingStore.enableDrawing('row')
  rows.value.forEach((row) => {
    row.keySequence.forEach((key) => matrixDrawingStore.addKeyToSequence(key))
    matrixDrawingStore.completeSequence()
  })
  matrixDrawingStore.enableDrawing('column')
  cols.value.forEach((col) => {
    col.keySequence.forEach((key) => matrixDrawingStore.addKeyToSequence(key))
    matrixDrawingStore.completeSequence()
  })
