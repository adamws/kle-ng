  ...
  return [
    {
      ...
      // layout, the same way Legend Tools and Move Rotation Origins do. Disabled while the tool
      // is already open — the panel is floating rather than modal, so this entry stays reachable,
      // and re-opening over a live preview would strand the user's original layout.
      disabled: isPreview || keyboardStore.keys.length === 0 || curveLayoutStore.isActive,
      action: () => {
