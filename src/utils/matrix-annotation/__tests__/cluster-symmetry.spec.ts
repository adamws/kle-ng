...
function pairsByRowCol(result: ReturnType<typeof clusterSymmetryAnnotationAlgorithm.annotate>) {
  return result.assignments.map((a, i) => ({ i, row: a?.row, col: a?.col }))
}
  ...
  describe('symmetric layouts', () => {
    ...
    it('forces right-side cols to be totalCols-1-leftCol on a symmetric split layout', () => {
      ...
      const totalCols = Math.max(
        ...result.assignments.map((a) => (a?.col ?? -1)),
      ) + 1
