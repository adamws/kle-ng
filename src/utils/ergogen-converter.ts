...
/**
 * Converts a Keyboard layout to an ergogen config YAML string
 * This is the reverse operation of parseErgogenConfig
 * @param keyboard - The keyboard layout to convert
 * @returns YAML config string for ergogen
 */
export function keyboardToErgogenConfig(keyboard: Keyboard): string {
  if (keyboard.keys.length === 0) {
    throw new Error('Cannot convert empty keyboard layout to ergogen config')
  }

  // Get spacing units from metadata (if available) or use defaults
  const spacingUnitX = keyboard.meta.spacing_x || 19
  const spacingUnitY = keyboard.meta.spacing_y || 19

  // Detect standard key dimensions
  const widths: number[] = []
  const heights: number[] = []

  for (const key of keyboard.keys) {
    if (key.width !== undefined) widths.push(key.width)
    if (key.height !== undefined) heights.push(key.height)
  }

  // Find most common width and height
  let standardWidth = 18
  if (widths.length >= 2) {
    const widthCounts: Record<number, number> = {}
    for (const w of widths) {
      widthCounts[w] = (widthCounts[w] || 0) + 1
  ...
  // Convert KLE units back to mm
  const denormalizeX = (value: number): number => new Decimal(value).mul(spacingUnitX).toNumber()
  const denormalizeY = (value: number): number => new Decimal(value).mul(spacingUnitY).toNumber()
  const denormalizeWidth = (value: number): number => new Decimal(value).mul(standardWidth).toNumber()
  const denormalizeHeight = (value: number): number => new Decimal(value).mul(standardHeight).toNumber()

  // Convert keys to ergogen points
  const points: Record<string, ErgogenPoint> = {}

  // Find topmost and bottommost Y values in KLE for Y-axis flipping
  let topmostY = new Decimal(Infinity)
  let bottommostY = new Decimal(-Infinity)
  for (const key of keyboard.keys) {
    const keyHeight = key.height !== undefined ? key.height : standardHeight
    const keyTopY = new Decimal(key.y)
    const keyBottomY = keyTopY.plus(keyHeight)

    if (keyTopY.lessThan(topmostY)) {
      topmostY = keyTopY
    }
    if (keyBottomY.greaterThan(bottommostY)) {
      bottommostY = keyBottomY
    }
  }

  // Calculate the height of the keyboard in KLE units
  const keyboardHeight = bottommostY.minus(topmostY).toNumber()

  for (let i = 0; i < keyboard.keys.length; i++) {
    const key = keyboard.keys[i]!
    const keyName = key.labels[0] || `key_${i}`

    // Convert from corner-based to center-based coordinates
    const keyWidth = key.width !== undefined ? key.width : standardWidth
    const keyHeight = key.height !== undefined ? key.height : standardHeight

    const widthMm = denormalizeWidth(keyWidth)
    const heightMm = denormalizeHeight(keyHeight)

    // Convert KLE corner coordinates to center coordinates
    const centerX = new Decimal(key.x).plus(new Decimal(keyWidth).div(2)).toNumber()
    const centerY = new Decimal(key.y).plus(new Decimal(keyHeight).div(2)).toNumber()

    // Convert to mm
    const xMm = denormalizeX(centerX)
    // Flip Y-axis: KLE Y-down → ergogen Y-up
    // In KLE: Y increases downward (top keys have lower Y)
    // In ergogen: Y increases upward (top keys have higher Y)
    // Formula: ergogenY = keyboardHeight - (centerY - topmostY)
    const yOffsetFromTop = new Decimal(centerY).minus(topmostY.toNumber()).toNumber()
    const flippedY = keyboardHeight - yOffsetFromTop
    const yMm = denormalizeY(flippedY)

    // Get rotation (KLE and ergogen rotate in opposite directions)
    ...
    // Add meta information if key size differs from standard
    if (keyWidth !== standardWidth || keyHeight !== standardHeight) {
      point.meta = {
        width: widthMm,
        height: heightMm,
      }
  ...
  // Create minimal ergogen config structure
  // Note: Ergogen typically generates points from zone configurations (columns/rows),
  // but ergogen.xyz may support direct point definitions. We create a structure
  // that defines points directly within a zone, which ergogen.xyz might process.
  const ergogenConfig = {
    meta: {
      engine: '4.1.0',
    },
    points: {
      zones: {
        matrix: {
          // Define points directly - ergogen.xyz may support this format
          // even though standard ergogen generates points from columns/rows
          points: points,
        },
      },
    },
