...
describe('curve layout store', () => {
  ...
  it('re-opening over a live preview keeps the original as the thing Cancel restores', () => {
    // The toolbar entry is reachable while the floating panel is open. A second `begin()` used
    // to re-snapshot the previewed geometry as the original, and since the preview is not in
    // history either, Cancel could no longer reach the layout the user started from.
    const keyboard = useKeyboardStore()
    keyboard.keys = row(6)
    const before = keyboard.keys.map(geometryOf)
    ...
    // The re-entrant call left the preview alone, rather than re-solving from a straight spine.
