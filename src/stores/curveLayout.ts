...
export const useCurveLayoutStore = defineStore('curve-layout', () => {
   ...
   *
   * Re-entrant calls are ignored. Opening a second time over a live preview would snapshot the
   * *previewed* geometry as the original, so Cancel could no longer reach the layout the user
   * started from — and since nothing has been committed to history yet, nothing else could
   * either. The toolbar entry is disabled while the tool is open; this is the backstop.
   */
  const begin = () => {
